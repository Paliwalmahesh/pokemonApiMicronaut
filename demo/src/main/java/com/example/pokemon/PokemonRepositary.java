...
public class PokemonRepositary {
    ...
    public Pokemon getpokemon(Long id) {
        return pokemonList.stream().filter(pokemon -> pokemon.getId().equals(id)).findFirst().orElse(null);
    }

    public List<Pokemon> updatePokemonByID(Pokemon pokemon, Long id) {
        Pokemon updatedPokemon=pokemonList.stream().filter(pokemonUpdate -> pokemonUpdate.getId().equals(id)).findFirst().orElseThrow();
    ...
    public List<Pokemon> deletePokemon(Long id) {
        Pokemon deletePokemon=pokemonList.stream().filter(pokemon -> pokemon.getId().equals(id)).findFirst().orElse(null);
